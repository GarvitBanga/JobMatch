...
class JobExtractor {
    constructor() {
        ...
        this.maxJobsToFetch = 25; // Increased from 5 to 25 for better coverage
        this.fetchTimeout = 10000; // 10 seconds
        this.batchProcessing = true; // Enable batch processing for OpenAI
        this.concurrentFetches = 5; // Process 5 jobs at once to speed up
    }
    ...
    async extractJobListings() {
        ...
        console.log('🔍 Starting enhanced job extraction with batch processing...');
        
        // Step 1: Find job links on the page
        const jobLinks = await this.findJobLinks();
        
        if (jobLinks.length > 0) {
            console.log(`📋 Found ${jobLinks.length} job links, fetching up to ${this.maxJobsToFetch} detailed descriptions...`);
            
            // Process more jobs but still limit to prevent timeout
            const limitedLinks = jobLinks.slice(0, this.maxJobsToFetch);
            
            // Step 2: Fetch detailed job info with concurrent processing
            const jobs = await this.fetchJobsBatch(limitedLinks);
            
            console.log(`🎯 Final result: ${jobs.length} jobs extracted for batch analysis`);
            return jobs;
        }
    ...
    // New method: Batch job fetching with concurrency control
    async fetchJobsBatch(jobLinks) {
        const jobs = [];
        const batches = [];
        
        // Split into concurrent batches
        for (let i = 0; i < jobLinks.length; i += this.concurrentFetches) {
            batches.push(jobLinks.slice(i, i + this.concurrentFetches));
        }
        
        console.log(`🚀 Processing ${jobLinks.length} jobs in ${batches.length} batches of ${this.concurrentFetches}`);
        ...
        for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
            ...
            const batchPromises = batch.map(async (link, index) => {
                try {
                    ...
                    if (fullJob && fullJob.title && fullJob.description && fullJob.description.length > 100) {
                        console.log(`✅ Batch ${batchIndex + 1}, Job ${index + 1}: ${fullJob.title}`);
                        return fullJob;
                    }
                } catch (error) {
                    console.error(`❌ Batch ${batchIndex + 1}, Job ${index + 1} failed:`, error);
        ...
        console.log(`📊 Batch processing complete: ${jobs.length}/${jobLinks.length} jobs successfully extracted`);
        return jobs;
    }
...
async function extractPageContent() {
    ...
    const content = {
        ...
        extractionMetadata: {
            maxJobsToFetch: extractor.maxJobsToFetch,
            concurrentFetches: extractor.concurrentFetches,
    ...
    try {
        ...
        console.log(`📊 Batch processing enabled for ${enhancedJobs.length > 3 ? 'YES' : 'NO'} (threshold: 3+ jobs)`);
        ...
        content.jobElements = enhancedJobs.map((job, index) => ({
            ...
            url: job.url,
            extractionMethod: job.extractionMethod || 'enhanced'
        }));
        
        // Update extraction metadata
        content.extractionMetadata.jobsExtracted = enhancedJobs.length;
        content.extractionMetadata.batchEligible = enhancedJobs.length > 3;
        content.extractionMetadata.extractionSuccess = true;
        ...
        // Update extraction metadata for fallback
        content.extractionMetadata.jobsExtracted = basicJobs.length;
        content.extractionMetadata.batchEligible = false;
        content.extractionMetadata.extractionSuccess = false;
        content.extractionMetadata.extractionMethod = 'basic_fallback';
        content.batchProcessing = false; // Disable batch for basic extraction
    }
    ...
    console.log(`📊 Final extraction summary:`, {
        ...
        batchEligible: content.extractionMetadata.batchEligible,
